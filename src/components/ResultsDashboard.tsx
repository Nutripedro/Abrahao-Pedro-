...
export const ResultsDashboard: React.FC<ResultsDashboardProps> = ({
  ...
  return (
    <div id="card-resultado-avaliacao" className="space-y-4">
      {/* 1. Resultado Principal: % Gordura Corporal e Métricas Chave */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {/* % Gordura Corporal em Destaque */}
        <div className="bg-slate-900 text-white p-6 rounded-3xl border border-sky-900/60 relative overflow-hidden shadow-xs md:col-span-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-3">
          {/* IMC */}
          <div className="bg-white p-4.5 rounded-3xl border border-slate-200/90 shadow-xs flex flex-col justify-between">
            ...
            <span className="mt-2 block text-[11px] font-medium text-emerald-700">
          ...
          <div className="bg-white p-4.5 rounded-3xl border border-slate-200/90 shadow-xs flex flex-col justify-between">
            ...
            <span className="mt-2 block text-[11px] text-slate-500 truncate" title="Harris-Benedict (Roza & Shizgal, 1984)">
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-sky-50 text-sky-700 border border-sky-100 flex items-center justify-center shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-amber-50/40 rounded-2xl border border-amber-200/80 p-5 flex flex-col justify-between space-y-4">
            <div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-amber-500 ring-4 ring-amber-100 shrink-0"></span>
              ...
              <div className="mt-3 flex items-baseline gap-2">
                <span className="text-3xl sm:text-4xl font-black font-mono text-amber-950 tabular-nums">
                  {results.massaGordaKg.toFixed(2)}
                </span>
                <span className="text-lg font-bold font-mono text-amber-800">kg</span>
              ...
              <div className="mt-4 p-3 rounded-xl bg-white/90 border border-amber-200/70 text-xs font-mono space-y-1">
                <span className="text-[10px] text-amber-800 font-bold uppercase block tracking-wider">
                  Cálculo da Massa Gorda:
                </span>
                <div className="text-slate-700 font-medium leading-relaxed">
                  <span className="text-amber-900 font-bold">MG (kg)</span> = Peso Total × (% Gordura / 100)
                </div>
                <div className="text-amber-950 font-bold pt-0.5">
          ...
          <div className="bg-sky-50/40 rounded-2xl border border-sky-200/80 p-5 flex flex-col justify-between space-y-4">
            <div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-sky-600 ring-4 ring-sky-100 shrink-0"></span>
                  <span className="text-xs font-bold uppercase tracking-wider text-sky-900 font-mono">
                    Massa Livre de Gordura (Massa Magra - MLG)
                  </span>
                </div>
                <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-full bg-sky-100 text-sky-900 border border-sky-300">
              ...
              {/* Valor Grande em Quilos */}
              <div className="mt-3 flex items-baseline gap-2">
                <span className="text-3xl sm:text-4xl font-black font-mono text-sky-950 tabular-nums">
                  {results.massaLivreGorduraKg.toFixed(2)}
                </span>
                <span className="text-lg font-bold font-mono text-sky-800">kg</span>
                <span className="text-xs text-sky-700 font-mono ml-auto">
                  (~{results.massaLivreGorduraKg.toFixed(1)} kg)
                </span>
              </div>

              {/* Memória de Cálculo Explícita */}
              <div className="mt-4 p-3 rounded-xl bg-white/90 border border-sky-200/70 text-xs font-mono space-y-1">
                <span className="text-[10px] text-sky-800 font-bold uppercase block tracking-wider">
                  Cálculo da Massa Livre de Gordura:
                </span>
                <div className="text-slate-700 font-medium leading-relaxed">
                  <span className="text-sky-900 font-bold">MLG (kg)</span> = Peso Total - Massa Gorda
                </div>
                <div className="text-sky-950 font-bold pt-0.5">
        ...
        <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 space-y-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between text-xs gap-1.5 font-sans">
            ...
            <span className="font-mono text-slate-600 text-[11px]">
              Verificação: <strong className="text-sky-800">{results.massaLivreGorduraKg.toFixed(1)} kg</strong> (Massa Magra) + <strong className="text-amber-800">{results.massaGordaKg.toFixed(1)} kg</strong> (Massa Gorda) = <strong className="text-slate-900">{peso.toFixed(1)} kg</strong> (100%)
            </span>
          </div>

          {/* Barra Visual Segmentada de Composição Corporal */}
          <div className="w-full h-8 bg-slate-200 rounded-xl overflow-hidden flex shadow-inner">
            <div
              style={{ width: `${leanPct}%` }}
          ...
          <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-500 font-mono pt-1">
            <div className="flex items-center gap-4">
              <span className="flex items-center gap-1.5">
                ...
                <span>Massa Magra: <strong className="text-slate-700">{results.massaLivreGorduraKg.toFixed(2)} kg ({leanPct}%)</strong></span>
              </span>
              <span className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-amber-500"></span>
                <span>Massa Gorda: <strong className="text-slate-700">{results.massaGordaKg.toFixed(2)} kg ({fatPct}%)</strong></span>
              </span>
            </div>
            <span>
              Razão MLG / MG: <strong className="text-slate-800">{(results.massaLivreGorduraKg / Math.max(results.massaGordaKg, 0.01)).toFixed(2)} : 1</strong>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200/90 shadow-xs flex flex-col items-center justify-center">
          <span className="text-xs font-bold text-slate-900 uppercase tracking-tight mb-3 font-sans">
          ...
          <div className="relative w-36 h-36 flex items-center justify-center">
            ...
            <div className="absolute flex flex-col items-center justify-center text-center">
              ...
              <span className="text-[10px] text-slate-500 uppercase font-mono">Gordura</span>
            </div>
          </div>
          ...
          <div className="w-full mt-4 pt-3 border-t border-slate-100 flex items-center justify-around text-xs">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-slate-600">Massa Magra: <strong>{leanPct}%</strong></span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-amber-500"></span>
              <span className="text-slate-600">Gordura: <strong>{fatPct}%</strong></span>
            </div>
          </div>
        </div>

        {/* Gráfico de Barras Visuais Proporcionais */}
        <div className="md:col-span-2 bg-white p-6 rounded-3xl border border-slate-200/90 shadow-xs flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-mono text-slate-500">
                Peso: <strong>{peso.toFixed(1)} kg</strong>
              </span>
            </div>
            ...
            <div className="space-y-2.5 text-xs">
              ...
              <div>
                <div className="flex justify-between text-slate-700 mb-1">
                  <span className="font-semibold text-sky-800">MASSA LIVRE DE GORDURA (MAGRA)</span>
              ...
              <div>
                <div className="flex justify-between text-slate-700 mb-1">
                  <span className="font-semibold text-amber-800">MASSA GORDA</span>
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 flex flex-wrap items-center justify-between text-[11px] text-slate-500 font-mono">
            <span>Soma das Dobras: <strong className="text-slate-800">{results.somaDobras.toFixed(1)} mm</strong></span>
            <span>Densidade Corporal: <strong className="text-slate-800">{results.densidadeCorporal.toFixed(5)} g/cm³</strong></span>
          </div>
        </div>
      </div>

      {/* Bento Sub-Tile: Memória de Cálculo / Fórmulas Transparentes */}
      <div className="p-6 rounded-3xl bg-slate-900 text-slate-200 border border-slate-800 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="inline-flex rounded-full shadow-inner border border-slate-700/80 bg-slate-950 p-0.5">
              <button
                type="button"
