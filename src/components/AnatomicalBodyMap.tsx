...
export const AnatomicalBodyMap: React.FC<AnatomicalBodyMapProps> = ({
  ...
  return (
    <div id="card-mapa-corporal" className="bg-white rounded-3xl border border-slate-200/90 shadow-xs p-6 transition-all flex flex-col h-full">
      <div className="flex items-center justify-between pb-4 mb-4 border-b border-slate-100">
        <div>
          ...
          <span className="text-xs text-slate-500">Localização anatômica das dobras</span>
        </div>
        <span className="text-[11px] font-mono font-medium px-3 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200">
      ...
      <div className="flex items-center justify-center gap-4 text-[11px] text-slate-600 mb-3 py-2 px-3 bg-slate-50/80 rounded-2xl border border-slate-200/80">
      ...
      <div className="relative flex-1 min-h-[360px] bg-slate-50/60 rounded-2xl border border-slate-200/80 p-3 flex items-center justify-center overflow-hidden">
        <div className="grid grid-cols-2 gap-2 w-full max-w-sm">
          {/* VISTA ANTERIOR (FRONT) */}
          <div className="flex flex-col items-center">
            <span className="text-[10px] font-bold tracking-wider text-slate-600 uppercase mb-1 font-mono">
      ...
      <div className="mt-4 p-4 bg-sky-50/50 rounded-2xl border border-sky-100 min-h-[105px] flex flex-col justify-between">
        {activeDef ? (
          <div>
            <div className="flex items-center justify-between gap-2 mb-1.5">
              <div className="flex items-center gap-2">
                <span className="w-5 h-5 rounded-md bg-slate-900 text-white font-mono text-[11px] font-bold flex items-center justify-center">
                  {activeDef.number}
                </span>
                <strong className="text-xs font-bold text-slate-900 font-sans">
                  {activeDef.name}
                </strong>
                <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded-full bg-white text-slate-700 border border-slate-200">
              ...
              {skinfoldValues[activeDef.key] && Number(skinfoldValues[activeDef.key]) > 0 ? (
                ...
                <span className="text-[11px] font-mono text-amber-700 bg-amber-50 px-2.5 py-0.5 rounded-full border border-amber-200">
                  Pendente
                </span>
              )}
            </div>
            ...
            <p className="text-[11px] text-slate-700 leading-relaxed">
              <strong className="text-slate-900">Localização:</strong> {activeDef.anatomicalLocation}
            </p>
            ...
            {onSwitchToQuickTip && (
              <div className="mt-3 pt-2.5 border-t border-sky-100 flex items-center justify-between">
                <span className="text-[10px] text-slate-500">Dúvida no pinçamento ou ângulo?</span>
                <button
                  type="button"
                  onClick={onSwitchToQuickTip}
                  className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-bold text-amber-900 dark:text-amber-300 bg-amber-100/80 hover:bg-amber-200/80 rounded-lg transition-colors cursor-pointer"
                >
          ...
          <div className="h-full flex items-center justify-center text-center text-xs text-slate-600 py-3">
            <Info className="w-4 h-4 mr-1.5 text-sky-700 shrink-0" />
